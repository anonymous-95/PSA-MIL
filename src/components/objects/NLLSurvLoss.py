...
class NLLSurvLoss(nn.Module):
    ...
    def __init__(self, alpha=0.5, eps=1e-7, reduction='mean'):
        super().__init__()
        self.alpha = alpha
        self.eps = eps
    ...
    @staticmethod
    def functional(h, y, t, c):
        """
        :param h: bin logits - (batch_size, n_bins)
        :param y: true y bin - (batch_size, n_bins)
        :param t: true times (batch_size,)
        :param c: censorship (batch_size,)
        :return:
        """
        return nll_loss(h=h, y=y.unsqueeze(dim=1), c=c.unsqueeze(dim=1))
